...
template <typename ServiceT,
          typename SharedRequest = typename ServiceT::Request::SharedPtr,
          typename SharedResponse = typename ServiceT::Response::SharedPtr>
int UFRobotSystemHardware::_call_request(std::shared_ptr<ServiceT> client,
                                         SharedRequest req,
                                         SharedResponse &res) {
  ...
  while (!client->wait_for_service(std::chrono::seconds(1))) {
    if (!rclcpp::ok()) {
      RCLCPP_ERROR(LOGGER,
                   "[%s] Interrupted while waiting for the service. Exiting.",
                   robot_ip_.c_str());
      exit(1);
    }
    if (!is_try_again) {
      is_try_again = true;
      RCLCPP_WARN(LOGGER, "[%s] service %s not available, waiting ...",
                  robot_ip_.c_str(), client->get_service_name());
    }
    failed_cnts += 1;
    if (failed_cnts >= 5)
      return WAIT_SERVICE_TIMEOUT;
  }
  auto result_future = client->async_send_request(req);
  if (rclcpp::spin_until_future_complete(hw_node_, result_future,
                                         std::chrono::seconds(1)) !=
      rclcpp::FutureReturnCode::SUCCESS) {
    // RCLCPP_ERROR(LOGGER, "[%s] Failed to call service %s", robot_ip_.c_str(),
...
void UFRobotSystemHardware::_init_ufactory_driver(void) {
  ...
  RCLCPP_INFO(LOGGER, "[%s] namespace: %s", robot_ip_.c_str(),
              node_->get_namespace());
  RCLCPP_INFO(LOGGER,
              "[%s] robot_type: %s, hw_ns: %s, prefix: %s, report_type: %s",
              robot_ip_.c_str(), robot_type.c_str(), hw_ns.c_str(),
              prefix.c_str(), report_type.c_str());
  ...
  node_->set_parameter(
      rclcpp::Parameter("default_gripper_baud", default_gripper_baud));
  ...
  RCLCPP_INFO(LOGGER,
              "[%s] dof: %d, velocity_control: %d, add_gripper: %d, "
              "baud_checkset: %d, default_gripper_baud: %d",
              robot_ip_.c_str(), dof, velocity_control_, add_gripper,
              baud_checkset, default_gripper_baud);

  xarm_driver_.init(node_, robot_ip_);
}

CallbackReturn
UFRobotSystemHardware::on_init(const hardware_interface::HardwareInfo &info) {
  ...
  velocity_states_.resize(info_.joints.size(),
                          std::numeric_limits<double>::quiet_NaN());
  position_cmds_.resize(info_.joints.size(),
                        std::numeric_limits<double>::quiet_NaN());
  velocity_cmds_.resize(info_.joints.size(),
                        std::numeric_limits<double>::quiet_NaN());

  for (const hardware_interface::ComponentInfo &joint : info_.joints) {
    bool has_pos_cmd_interface = false;
    for (auto i = 0u; i < joint.command_interfaces.size(); ++i) {
      if (joint.command_interfaces[i].name ==
    ...
    if (!has_pos_cmd_interface) {
      RCLCPP_ERROR(LOGGER,
                   "[%s] Joint '%s' has %ld command interfaces found, but not "
                   "found %s command interface",
                   robot_ip_.c_str(), joint.name.c_str(),
    ...
    for (auto i = 0u; i < joint.state_interfaces.size(); ++i) {
      if (joint.state_interfaces[i].name ==
          hardware_interface::HW_IF_POSITION) {
    ...
    if (!has_pos_state_interface) {
      RCLCPP_ERROR(LOGGER,
                   "[%s] Joint '%s' has %ld state interfaces found, but not "
                   "found %s state interface",
                   robot_ip_.c_str(), joint.name.c_str(),
...
UFRobotSystemHardware::export_state_interfaces() {
...
UFRobotSystemHardware::export_command_interfaces() {
...
CallbackReturn UFRobotSystemHardware::on_activate(
  ...
  xarm_driver_.arm->set_mode(velocity_control_ ? XARM_MODE::VELO_JOINT
                                               : XARM_MODE::SERVO);
  xarm_driver_.arm->set_state(XARM_STATE::START);

  req_list_controller_ = std::make_shared<
      controller_manager_msgs::srv::ListControllers::Request>();
  res_list_controller_ = std::make_shared<
      controller_manager_msgs::srv::ListControllers::Response>();
  req_switch_controller_ = std::make_shared<
      controller_manager_msgs::srv::SwitchController::Request>();
  res_switch_controller_ = std::make_shared<
      controller_manager_msgs::srv::SwitchController::Response>();

  client_list_controller_ =
      hw_node_->create_client<controller_manager_msgs::srv::ListControllers>(
          "/controller_manager/list_controllers");
  client_switch_controller_ =
      hw_node_->create_client<controller_manager_msgs::srv::SwitchController>(
...
CallbackReturn UFRobotSystemHardware::on_deactivate(
    const rclcpp_lifecycle::State &previous_state) {
...
UFRobotSystemHardware::read(const rclcpp::Time &time,
  ...
  if (use_new)
    read_code_ = xarm_driver_.arm->get_joint_states(
        curr_read_position_, curr_read_velocity_, curr_read_effort_);
  else
  ...
  //     RCLCPP_INFO(LOGGER, "[%s] [READ] cnt: %ld, max: %f, mean: %f, failed:
  //     %ld", robot_ip_.c_str(), read_cnts_, read_max_time_, read_total_time_ /
  //     read_cnts_, read_failed_cnts_);
  // }
  if (read_code_ == 0 && read_ready_) {
    for (int j = 0; j < info_.joints.size(); j++) {
      ...
      if (use_new) {
        ...
        velocity_states_[j] =
            !initialized_
                ? 0.0
                : (curr_read_position_[j] - prev_read_position_[j]) /
                      (curr_read_time_.seconds() - prev_read_time_.seconds());
    ...
    if (!initialized_) {
      ...
      if (reload_controller_ &&
          _check_cmds_is_change(curr_read_position_, prev_read_position_)) {
        _reload_controller();
      }
    }
    ...
    if (read_code_) {
      read_failed_cnts_ += 1;
      RCLCPP_INFO(LOGGER, "[%s] Read() returns: %d", robot_ip_.c_str(),
                  read_code_);
      if (read_code_ == ROBOT_IS_DISCONNECTED) {
        RCLCPP_ERROR(LOGGER, "[%s] Robot is disconnected, ros shutdown",
                     robot_ip_.c_str());
        rclcpp::shutdown();
        exit(1);
      }
...
hardware_interface::return_type
UFRobotSystemHardware::write(const rclcpp::Time &time,
                             const rclcpp::Duration &period) {
  if (_need_reset()) {
    if (initialized_)
      reload_controller_ = true;
    initialized_ = false;
    return hardware_interface::return_type::OK;
  }
  initialized_ = true;
  ...
  // pos_str += "]";
  // vel_str += "]";
  // RCLCPP_INFO(LOGGER, "[%s] positon: %s, velocity: %s", robot_ip_.c_str(),
  // pos_str.c_str(), vel_str.c_str());
  ...
  if (velocity_control_) {
    ...
    // RCLCPP_INFO(LOGGER, "[%s] velocity: %s", robot_ip_.c_str(),
    // vel_str.c_str());
    cmd_ret = xarm_driver_.arm->vc_set_joint_velocity(cmds_float_, true,
                                                      VELO_DURATION);
    if (cmd_ret != 0) {
      RCLCPP_WARN(LOGGER, "[%s] vc_set_joint_velocity, ret=%d",
                  robot_ip_.c_str(), cmd_ret);
    }
  } else {
    for (int i = 0; i < position_cmds_.size(); i++) {
    ...
    if (curr_write_time_.seconds() - prev_write_time_.seconds() > 1 ||
        _check_cmds_is_change(prev_cmds_float_, cmds_float_)) {
      // RCLCPP_INFO(LOGGER, "[%s] positon: %s", robot_ip_.c_str(),
      // pos_str.c_str());
      cmd_ret = xarm_driver_.arm->set_servo_angle_j(cmds_float_, 0, 0, 0);
      if (cmd_ret != 0) {
        RCLCPP_WARN(LOGGER, "[%s] set_servo_angle_j, ret= %d",
                    robot_ip_.c_str(), cmd_ret);
      }
      if (cmd_ret == 0) {
...
void UFRobotSystemHardware::_reload_controller(void) {
  int ret = _call_request(client_list_controller_, req_list_controller_,
                          res_list_controller_);
  if (ret == 0 && res_list_controller_->controller.size() > 0) {
    req_switch_controller_->activate_controllers.resize(
        res_list_controller_->controller.size());
    req_switch_controller_->deactivate_controllers.resize(
        res_list_controller_->controller.size());
    for (uint i = 0; i < res_list_controller_->controller.size(); i++) {
      req_switch_controller_->activate_controllers[i] =
          res_list_controller_->controller[i].name;
      req_switch_controller_->deactivate_controllers[i] =
          res_list_controller_->controller[i].name;
    }
    req_switch_controller_->strictness =
        controller_manager_msgs::srv::SwitchController::Request::BEST_EFFORT;
    _call_request(client_switch_controller_, req_switch_controller_,
...
bool UFRobotSystemHardware::_check_cmds_is_change(float *prev, float *cur,
                                                  double threshold) {
  for (int i = 0; i < 7; i++) {
    if (std::abs(cur[i] - prev[i]) > threshold)
      return true;
  }
  return false;
}
...
bool UFRobotSystemHardware::_xarm_is_ready_read(void) {
  ...
  if (curr_err != 0) {
    if (last_err != curr_err) {
      RCLCPP_ERROR(LOGGER, "[%s] UFACTORY Error detected! Code C%d -> [ %s ] ",
                   robot_ip_.c_str(), curr_err,
                   xarm_driver_.controller_error_interpreter(curr_err).c_str());
    }
  }
  last_err = curr_err;
  return last_err == 0;
...
bool UFRobotSystemHardware::_xarm_is_ready_write(void) {
  ...
  if (curr_state > 2) {
    if (last_state != curr_state) {
      ...
      RCLCPP_ERROR(LOGGER, "[%s] Robot State detected! State: %d",
  ...
  if (!(velocity_control_ ? curr_mode == XARM_MODE::VELO_JOINT
                          : curr_mode == XARM_MODE::SERVO)) {
    if (last_mode != curr_mode) {
      last_mode = curr_mode;
      RCLCPP_ERROR(LOGGER, "[%s] Robot Mode detected! Mode: %d",
...
bool UFRobotSystemHardware::_firmware_version_is_ge(int major, int minor,
                                                    int revision) {
  return xarm_driver_.arm->version_number[0] > major ||
         (xarm_driver_.arm->version_number[0] == major &&
          xarm_driver_.arm->version_number[1] > minor) ||
         (xarm_driver_.arm->version_number[0] == major &&
          xarm_driver_.arm->version_number[1] == minor &&
          xarm_driver_.arm->version_number[2] >= revision);
}
...
bool UFRobotSystemHardware::_need_reset() {
  ...
  if (!write_succeed) {
    ...
    RCLCPP_ERROR(LOGGER,
                 "[%s] Write() failed, failed_ret=%d !, Setting Robot State to "
                 "STOP... (ret: %d)",
                 robot_ip_.c_str(), write_code_, ret);
    if (write_code_ == SERVICE_IS_PERSISTENT_BUT_INVALID ||
        write_code_ == SERVICE_CALL_FAILED) {
      RCLCPP_ERROR(LOGGER, "[%s] Service is invaild, ros shutdown",
                   robot_ip_.c_str());
      rclcpp::shutdown();
      exit(1);
